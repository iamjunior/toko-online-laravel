...
<div class="row">
    <div class="col-md-8">
        ...
        <form enctype="multipart/form-data" method="POST" action="{{route('books.update', [$book->id])}}" class="p-3 shadow-sm bg-white" >
            ...
            <input type="text" class="form-control {{$errors->first('title') ? "is-invalid" : ""}}"  value="{{old('name') ? old('name') : $book->title}}" name="title" placeholder="Book title" />
            ...
            <input type="text" class="form-control {{$errors->first('stock') ? "is-invalid" : ""}}" placeholder="Stock" id="stock" name="stock" value="{{old('phone') ? old('phone') : $book->stock}}">
            <div class="invalid-feedback">
                {{$errors->first('stock')}}
            </div>
            <br>
...
<script>
    $('#categories').select2({
        ajax: {
        url: 'http://larashop.dumi/ajax/categories/search',
